...
app.post(`${apiBase}`, async (req, res) => {
    const {datareuniaoStr, dirigidapor, presididapor, primeiro_hino, primeira_oracao, mensagem, ultima_oracao, numero, anuncio, observacao, primeiro_hino_nome} = req.body;

  const sql = 'INSERT INTO reuniao (datareuniao, dirigidapor, presididapor, primeiro_hino, primeira_oracao, mensagem, ultima_oracao, numero, anuncio, observacao, primeiro_hino_nome) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';

  try{
    await req.db.query(sql, [datareuniaoStr, dirigidapor, presididapor, primeiro_hino, primeira_oracao, mensagem, ultima_oracao, numero, anuncio, observacao, primeiro_hino_nome], (err, result) => {
      if (err) {
...
app.put(`${apiBase}:id`, async (req, res) => {
    ...
    const {dirigidapor, presididapor, primeiro_hino, primeira_oracao, mensagem, ultima_oracao, numero, anuncio, observacao, primeiro_hino_nome} = req.body;
    const sql = 'UPDATE reuniao SET dirigidapor = ?, presididapor = ?, primeiro_hino = ?, primeira_oracao = ?, mensagem = ?, ultima_oracao = ?, numero = ?, anuncio = ?, observacao = ?, primeiro_hino_nome = ?, modifiedon = ? WHERE Id = ?';

    const modifiedon = new Date();

    try{
      await req.db.query(sql, [dirigidapor, presididapor, primeiro_hino, primeira_oracao, mensagem, ultima_oracao, numero, anuncio, observacao, primeiro_hino_nome, modifiedon, id], (err, result) => {
        if (err) {
...
app.listen(port, () => {
  console.log(`db.reuniaofamiliar v0.0.1 - ${port}`);
});
